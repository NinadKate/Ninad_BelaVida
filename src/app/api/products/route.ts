...
function syncRegionalPrices(
  ...
  return Object.fromEntries(
    Object.entries(prices as Record<string, unknown>).map(([key, value]) => {
      const normalizedValue =
        value === null || value === undefined ? "" : String(value);

      if (normalizedValue === "" || normalizedValue === previousPrice) {
        return [key, nextPrice];
      }
...
export async function PUT(req: Request) {
  ...
  try {
    ...
    if (price !== undefined) {
      ...
      updates.prices = syncRegionalPrices(
        prices !== undefined ? prices : existingProduct.prices,
