...
import { formatCurrency, getRegionalPrice } from "@/lib/utils";
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-xl font-bold font-heading text-neutral-dark dark:text-transparent dark:bg-clip-text dark:bg-linear-to-r dark:from-white dark:to-neutral-400">
          {t("title")}
        </h2>
        <button
          onClick={() => {
        ...
        <table className="w-full text-left">
          <thead className="bg-neutral-soft dark:bg-white/5 border-b border-neutral-med dark:border-white/5 text-neutral-dark dark:text-neutral-200">
            <tr>
              ...
              <th className="p-4 text-sm font-bold text-right">
                {t("actions")}
              </th>
            </tr>
          </thead>
          <tbody className="divide-y divide-neutral-med dark:divide-white/5">
            {products.map((product) => {
              ...
              return (
                <tr
                  key={product.id}
                  className="hover:bg-neutral-50 dark:hover:bg-white/5 transition-all duration-300"
                >
                  ...
                  <td
                    className="p-4 text-sm font-bold text-neutral-dark dark:text-neutral-200"
                    suppressHydrationWarning
                  >
                    {formatCurrency(
                      getRegionalPrice(product, locale).amount,
                      getRegionalPrice(product, locale).currency,
                      locale,
                    )}
                  </td>
